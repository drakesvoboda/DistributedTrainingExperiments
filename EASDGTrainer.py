...
class ReducerThread(threading.Thread):
    def __init__(self, num_trainers: int, num_shards: int, max_in_flight: int, model: torch.nn.Module):
        ...
        with torch.no_grad():
            for key, p in self.params.items():
                fetched = remote_method(P3ParameterServer.fetch_param, self.ps_rref_map[key], key)
                p.copy_(fetched)
    ...
    def step(self):
        ...
        work = rpc.rpc_async(self.ps_rref_map[param_idx].owner(), P3ParameterServer.easgd_update, args=(self.ps_rref_map[param_idx], param_idx, self.params[param_idx].data))
        fut.set_result(work)
        self.in_flight.append(work)

class EASGDTrainer(Trainer):
    def __init__(self, model: torch.nn.Module, criterion: callable, optim_fn: callable, rank: int, world_size: int, tau: int):
        super().__init__(model, criterion, None)

        mp.spawn(run_parameter_server, nprocs=1, args=(args.world_size*2,args.world_size+rank,rank))

        rpc.init_rpc(name=f"trainer_{rank}", rank=rank, world_size=args.world_size*2)
        ...
        for idx, (module_name, module) in enumerate(model.named_modules()):
            ...
            module.updates = None
            module.iteration = idx
            module.optimizer = self.optim_fn(module.parameters(recurse=False))
            module.register_full_backward_hook(P3Trainer.backwards_pass_hook(self.reducer, self.tau, module_name))
            module.register_forward_pre_hook(P3Trainer.forward_pre_hook(num_trainers, module_name))
